/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   ft_lstadd_back_bonus.c                             :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: mmirdama <mmirdama@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/11/18 19:18:21 by mmirdama          #+#    #+#             */
/*   Updated: 2025/11/27 13:55:34 by mmirdama         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "libft.h"

void	ft_lstadd_back(t_list **lst, t_list *new)
{
	t_list	*curr_end;

	if (!lst || !new)
		return ;
	if (*lst == NULL)
	{
		*lst = new;
		return ;
	}
	curr_end = ft_lstlast(*lst);
	curr_end->next = new;
}

// #include <stdio.h>
// #include <stdlib.h>

// int	main(void)
// {
// 	char	*data1 = "Node 1";
// 	char	*data2 = "Node 2";
// 	char	*data3 = "Node 3";
// 	char	*data4 = "Node 4";

// 	t_list	*node_1;
// 	t_list	*node_2;
// 	t_list	*node_3;
// 	t_list	*node_4;
// 	t_list	*last_node;

// 	node_1 = ft_lstnew(data1);
// 	node_2 = ft_lstnew(data2);
// 	node_3 = ft_lstnew(data3);
// 	node_4 = ft_lstnew(data4);

// 	if (!node_1 || !node_2 || !node_3 || !node_4)
// 	{
// 		printf("malloc failed\n");
// 		return (1);
// 	}
// 	node_1->next = node_2;
// 	node_2->next = node_3;
// 	last_node = ft_lstlast(node_1);
// 	printf("last node of the list is at |%p| address\n", last_node);
// 	printf("last node content: |%s|\n", (char *)last_node->content);
// 	ft_lstadd_back(&node_1, node_4);
// 	last_node = ft_lstlast(node_1);
// 	printf("new last node of the list is at |%p| address\n", last_node);
// 	printf("New last node content: |%s|\n", (char *)last_node->content);
// 	free(node_1);
// 	free(node_2);
// 	free(node_3);
// 	free(node_4);
// 	return (0);
// }